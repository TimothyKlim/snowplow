...
class SplitBatchSpec extends WordSpec with Matchers {
  val splitBatch = SplitBatch

  "SplitBatch.split" should {
    "Batch a list of strings based on size" in {
      splitBatch.split(List("a", "b", "c"), 5, 1) should ===(
        SplitBatchResult(List(List("c"), List("b", "a")), Nil))
    }

    "Reject only those strings which are too big" in {
      splitBatch.split(List("123456", "1", "123"), 5, 0) should ===(
        SplitBatchResult(List(List("123", "1")), List("123456")))
    }

    "Batch a long list of strings" in {
      splitBatch.split(List("12345677890",
                            "123456789",
                            "12345678",
                            "1234567",
                            "123456",
                       ...
                       9,
                       0) should ===(
        SplitBatchResult(List(List("1", "12", "123"),
                              List("1234", "12345"),
                              List("123456"),
                              List("1234567"),
                              List("12345678"),
  ...
  "SplitBatch.splitAndSerializePayload" should {
    "Serialize an empty CollectorPayload" in {
      val actual =
        SplitBatch.splitAndSerializePayload(new CollectorPayload(), 100)
      val target = new CollectorPayload
      new TDeserializer().deserialize(target, actual.good.head)
      target should ===(new CollectorPayload)
    }
    ...
    "Split a CollectorPayload with three large events and four very large events" in {
      ...
      payload.setBody(data)
      val actual = SplitBatch.splitAndSerializePayload(payload, 1000)
      actual.bad.size should ===(4)
      actual.good.size should ===(2)
    }
  }
}
