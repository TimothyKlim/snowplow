...
object Shredder {
  ...
  def fixSchema(prefix: String,
    ...
    schema match {
      case schemaPattern(organization, name, schemaVer) => {
        ...
        val snakeCaseOrganization =
          organization.replaceAll("""[-.]""", "_").toLowerCase

        // Change the name from PascalCase to snake_case if necessary and replace hyphens with underscores
        val snakeCaseName =
          name.replaceAll("([^A-Z_])([A-Z])", "$1_$2").toLowerCase
      ...
      case _ =>
        "Schema %s does not conform to regular expression %s"
  ...
  def parseContexts(contexts: String): ValidationNel[String, JObject] = {
    ...
    def innerParseContexts(
      ...
      contextJsons match {
        ...
        case head :: tail => {
          val context = head
          val innerData: ValidationNel[String, JValue] =
            context \ "data" match {
              case JNothing =>
                "Could not extract inner data field from custom context".failureNel // TODO: decide whether to enforce object type of data
              case d => d.successNel
            }
          val fixedSchema: ValidationNel[String, String] =
            context \ "schema" match {
              case JString(schema) => fixSchema("contexts", schema)
              case _ =>
                "Context JSON did not contain a stringly typed schema field".failureNel
            }

          val schemaDataPair: ValidationNel[String, (String, JValue)] =
    ...
    data match {
      case JArray(Nil) => "Custom contexts data array is empty".failureNel
      case JArray(ls) => {
  ...
  def parseUnstruct(unstruct: String): ValidationNel[String, JObject] = {
    ...
    val innerData: ValidationNel[String, JValue] = data \ "data" match {
      case JNothing =>
        "Could not extract inner data field from unstructured event".failureNel // TODO: decide whether to enforce object type of data
      case d => d.successNel
    }
    val fixedSchema: ValidationNel[String, String] = schema match {
      case JString(s) => fixSchema("unstruct_event", s)
      case _ =>
        "Unstructured event JSON did not contain a stringly typed schema field".failureNel
    }
