...
class TransformerSpec
  ...
  "The 'fromClass' method" should {
    "successfully convert a tab-separated event string to JSON" in {
      ...
      // Specific fields
      ScalazJson4sUtils.extract[String](result, "platform") must beSuccessful(
        "web")
      ScalazJson4sUtils
        .extract[Int](result, "domain_sessionidx") must beSuccessful(3)
      ScalazJson4sUtils
        .extract[Double](result, "geo_latitude") must beSuccessful(37.443604)
      ScalazJson4sUtils
        .extract[Boolean](result, "br_features_pdf") must beSuccessful(true)
      ScalazJson4sUtils
        .extract[Boolean](result, "br_features_flash") must beSuccessful(false)
      ScalazJson4sUtils
        .extract[String](result, "collector_tstamp") must beSuccessful(
        "2013-11-26T00:02:05Z")
      ScalazJson4sUtils
        .extract[String](result, "geo_location") must beSuccessful(
        "37.443604,-122.4124")
      ScalazJson4sUtils.extract[String](result, "ti_sku") must beSuccessful(
        null)

      // Unstructured event shredding
      result \ "unstruct_event_com_snowplowanalytics_snowplow_link_click_1" \ "elementId" must_== JString(
        "exampleLink")

      // Contexts shredding
      result \ "contexts_org_schema_web_page_1" \ "genre" must_== JString(
        "blog")
