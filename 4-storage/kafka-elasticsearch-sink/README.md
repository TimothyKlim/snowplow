...
The Kafka Elasticsearch Sink consumes Snowplow enriched events or failed events from an [Kafka][kinesis] stream, transforms them to JSON, and writes them to [Elasticsearch][elasticsearch]. Events which cannot be transformed or which are rejected by Elasticsearch are written to a separate Kafka stream.
...
The Kafka Elasticsearch Sink has the following command-line interface:
