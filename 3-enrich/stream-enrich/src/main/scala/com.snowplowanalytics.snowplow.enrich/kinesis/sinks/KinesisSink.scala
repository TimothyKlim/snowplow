...
class KinesisSink(provider: AWSCredentialsProvider,
  ...
  def storeEnrichedEvents(events: List[(String, String)]): Boolean = {
    val wrappedEvents =
      events.map(e => ByteBuffer.wrap(e._1.getBytes(UTF_8)) -> e._2)
  ...
  def sendBatch(batch: List[(ByteBuffer, String)]): Unit = {
    if (!batch.isEmpty) {
      ...
      while (!sentBatchSuccessfully) {
        ...
        try {
          val results =
            Await.result(putData, 10.seconds).result.getRecords.asScala.toList
          ...
          if (failurePairs.nonEmpty) {
            ...
            val putSize: Long =
              unsentRecords.foldLeft(0)((a, b) => a + b._1.capacity)

            tracker match {
              case Some(t) =>
                SnowplowTracking.sendFailureEvent(t,
                                                  "PUT Failure",
                                                  err,
                                                  name,
