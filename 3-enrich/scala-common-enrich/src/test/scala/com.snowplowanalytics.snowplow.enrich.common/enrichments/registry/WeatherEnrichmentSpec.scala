...
class WeatherEnrichmentSpec extends Specification {
  ...
  def e1 = {
    val enr =
      WeatherEnrichment("KEY", 5200, 1, "history.openweathermap.org", 10)
    val stamp = enr.getWeatherContext(Option(invalidEvent.lat),
                                      Option(invalidEvent.lon),
                                      Option(invalidEvent.time))
    stamp.toEither must beLeft.like {
      case e => e must contain("tstamp: None")
    }
  }
  ...
  def e3 = {
    ...
    val stamp = enr
      .getWeatherContext(Option(validEvent.lat), Option(validEvent.lon), Option(validEvent.time))
    stamp.toEither must beLeft.like {
  ...
  def e4 = {
    val enr =
      WeatherEnrichment(validAppKey, 5200, 1, "history.openweathermap.org", 10)
    val stamp = enr
      .getWeatherContext(Option(validEvent.lat), Option(validEvent.lon), Option(validEvent.time))
  ...
  def e5 = {
    ...
    val stamp = enr
      .getWeatherContext(Option(validEvent.lat), Option(validEvent.lon), Option(validEvent.time))
    stamp.toEither must beLeft.like {
      case e => e must contain("AuthorizationError")
    }
  }

  def e6 = {
    val enr =
      WeatherEnrichment(validAppKey, 5200, 1, "history.openweathermap.org", 15)
    val stamp = enr
      .getWeatherContext(Option(validEvent.lat), Option(validEvent.lon), Option(validEvent.time))
    stamp.toEither must beRight.like {
      case weather: JValue => {
        val temp = weather.findField {
          case JField("humidity", _) => true; case _ => false
        }
  ...
  def e7 = {
    ...
    val config = WeatherEnrichmentConfig.parse(
      configJson,
      SchemaKey("com.snowplowanalytics.snowplow.enrichments",
  ...
  def e8 = {
    ...
    val enr =
      WeatherEnrichment(validAppKey, 2, 1, "history.openweathermap.org", 15)
    val stamp = enr
      .getWeatherContext(Option(validEvent.lat), Option(validEvent.lon), Option(validEvent.time))
    stamp.toEither must beRight.like { // successful request
      case weather: JValue => {
        ...
        e.map(_.dt) must beSome.like { // succesfull transformation
          case dt =>
            dt must equalTo("2015-08-29T23:16:14.000Z") // closest stamp storing on server
        }
      }
    }
  }
