...
class CurrencyConversionEnrichmentSpec extends Specification with DataTables {
  ...
  val invalidAppKeyFailure = Failure(NonEmptyList(
    "Open Exchange Rates error, type: [InvalidAppId], message: [Invalid App ID provided. Please sign up at https://openexchangerates.org/signup, or contact support@openexchangerates.org.]"))
  ...
  def e1 =
    "SPEC NAME" || "TRANSACTION CURRENCY" | "API KEY" | "TOTAL AMOUNT" | "TOTAL TAX" | "SHIPPING" | "TRANSACTION ITEM CURRENCY" | "TRANSACTION ITEM PRICE" | "DATETIME" | "CONVERTED TUPLE" |
      "Invalid transaction currency" !! Some("RUP") ! validAppKey ! Some(11.00) ! Some(
        1.17) ! Some(0.00) ! None ! Some(17.99) ! Some(coTstamp) ! currencyInvalidRup |
      "Invalid transaction item currency" !! None ! validAppKey ! Some(12.00) ! Some(
        0.7) ! Some(0.00) ! Some("HUL") ! Some(1.99) ! Some(coTstamp) ! currencyInvalidHul |
      "Invalid OER API key" !! None ! "8A8A8A8A8A8A8A8A8A8A8A8AA8A8A8A8" ! Some(
        13.00) ! Some(3.67) ! Some(0.00) ! Some("GBP") ! Some(2.99) ! Some(
        coTstamp) ! invalidAppKeyFailure |> {
      (_, trCurrency, apiKey, trAmountTotal, trAmountTax, trAmountShipping,
       tiCurrency, tiPrice, dateTime, expected) =>
        CurrencyConversionEnrichment(DeveloperAccount,
                                     apiKey,
                                     "EUR",
                                     "EOD_PRIOR").convertCurrencies(
          trCurrency,
          trAmountTotal,
          trAmountTax,
          trAmountShipping,
  ...
  def e2 =
    "SPEC NAME" || "TRANSACTION CURRENCY" | "API KEY" | "TOTAL AMOUNT" | "TOTAL TAX" | "SHIPPING" | "TRANSACTION ITEM CURRENCY" | "TRANSACTION ITEM PRICE" | "DATETIME" | "CONVERTED TUPLE" |
      ...
      "All fields absent except currency" !! Some("GBP") ! validAppKey ! None ! None ! None ! Some(
        "GBP") ! None ! None ! Failure(
        NonEmptyList("Collector timestamp missing")) |
      "No transaction currency, tax, or shipping" !! Some("GBP") ! validAppKey ! Some(
        11.00) ! None ! None ! None ! None ! Some(coTstamp) ! (Some("12.75"),
                                                               None,
                                                               None,
                                                               None).success |
      "No transaction currency or total" !! Some("GBP") ! validAppKey ! None ! Some(
        2.67) ! Some(0.00) ! None ! None ! Some(coTstamp) ! (None,
                                                             Some("3.09"),
                                                             Some("0.00"),
                                                             None).success |
      "No transaction currency" !! None ! validAppKey ! None ! None ! None ! Some(
        "GBP") ! Some(12.99) ! Some(coTstamp) ! (None,
                                                 None,
                                                 None,
                                                 Some("15.05")).success |
      "Transaction Item Null" !! Some("GBP") ! validAppKey ! Some(11.00) ! Some(
        2.67) ! Some(0.00) ! None ! None ! Some(coTstamp) ! (Some("12.75"),
                                                             Some("3.09"),
                                                             Some("0.00"),
                                                             None).success |
      "Valid APP ID and API key" !! None ! validAppKey ! Some(14.00) ! Some(4.67) ! Some(
        0.00) ! Some("GBP") ! Some(10.99) ! Some(coTstamp) ! (None,
                                                              None,
                                                              None,
                                                              Some("12.74")).success |
      "Both Currency Null" !! None ! validAppKey ! Some(11.00) ! Some(2.67) ! Some(
        0.00) ! None ! Some(12.99) ! Some(coTstamp) ! (None, None, None, None).success |
      "Convert to the same currency" !! Some("EUR") ! validAppKey ! Some(11.00) ! Some(2.67) ! Some(
        0.00) ! Some("EUR") ! Some(12.99) ! Some(coTstamp) ! (Some("11.00"),
                                                              Some("2.67"),
                                                              Some("0.00"),
                                                              Some("12.99")).success |
      "Valid APP ID and API key" !! Some("GBP") ! validAppKey ! Some(16.00) ! Some(
        2.67) ! Some(0.00) ! None ! Some(10.00) ! Some(coTstamp) ! (Some(
                                                                      "18.54"),
                                                                    Some(
                                                                      "3.09"),
                                                                    Some(
