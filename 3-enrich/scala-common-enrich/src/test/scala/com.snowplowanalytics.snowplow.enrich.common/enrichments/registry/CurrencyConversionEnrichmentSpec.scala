...
class CurrencyConversionEnrichmentSpec extends Specification with DataTables {
  ...
  def e1 =
    "SPEC NAME" || "TRANSACTION CURRENCY" | "API KEY" | "TOTAL AMOUNT" | "TOTAL TAX" | "SHIPPING" | "TRANSACTION ITEM CURRENCY" | "TRANSACTION ITEM PRICE" | "DATETIME" | "CONVERTED TUPLE" |
      ...
      (_, trCurrency, apiKey, trAmountTotal, trAmountTax, trAmountShipping, tiCurrency, tiPrice,
       dateTime, expected) =>
        CurrencyConversionEnrichment(DeveloperAccount, apiKey, "EUR", "EOD_PRIOR")
          .convertCurrencies(trCurrency,
                             trAmountTotal,
                             trAmountTax,
                             trAmountShipping,
                             tiCurrency,
  ...
  def e2 =
    "SPEC NAME" || "TRANSACTION CURRENCY" | "API KEY" | "TOTAL AMOUNT" | "TOTAL TAX" | "SHIPPING" | "TRANSACTION ITEM CURRENCY" | "TRANSACTION ITEM PRICE" | "DATETIME" | "CONVERTED TUPLE" |
      ...
      "Valid APP ID and API key" !! Some("GBP") ! validAppKey ! Some(16.00) ! Some(2.67) ! Some(
        0.00) ! None ! Some(10.00) ! Some(coTstamp) ! (Some("18.54"),
                                                       Some("3.09"),
                                                       Some("0.00"),
                                                       None).success |> {
      (_, trCurrency, apiKey, trAmountTotal, trAmountTax, trAmountShipping, tiCurrency, tiPrice,
