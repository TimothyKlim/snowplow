  ...
  "extractIpAddress" should {
    "correctly extract an X-FORWARDED-FOR header" in {
      ...
      "SPEC NAME" || "HEADERS" | "EXP. RESULT" |
        ...
        "No X-FORWARDED-FOR header" !! List(
          "Accept-Charset: utf-8",
          "Connection: keep-alive") ! Default |
        "Unparseable X-FORWARDED-FOR header" !! List(
          "X-Forwarded-For: localhost") ! Default |
        ...
        (_, headers, expected) =>
          {
            IpAddressExtractor
              .extractIpAddress(headers, Default) must_== expected
          }
      }
