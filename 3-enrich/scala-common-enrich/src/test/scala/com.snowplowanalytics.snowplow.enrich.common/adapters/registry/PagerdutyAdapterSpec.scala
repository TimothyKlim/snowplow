...
class PagerdutyAdapterSpec
  ...
  def e1 =
    "SPEC NAME" || "INPUT" | "EXPECTED OUTPUT" |
      ...
      (_, input, expected) =>
        PagerdutyAdapter.reformatParameters(parse(input)) mustEqual parse(expected)
    }
  ...
  def e4 = {
    ...
    val expected = List(
      JObject(
        List(("type", JString("incident.trigger")),
             ("data", JObject(List(("incident", JObject(List(("id", JString("P9WY9U9")))))))))))
