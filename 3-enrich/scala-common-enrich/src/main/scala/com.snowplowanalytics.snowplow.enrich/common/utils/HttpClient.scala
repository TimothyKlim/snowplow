...
class HttpClient(actorSystem: ActorSystem) {
  ...
  /**
    ...
    */
  private def buildAuthorization(authUser: Option[String],
                                 authPassword: Option[String]): Option[Authorization] =
    if (List(authUser, authPassword).flatten.isEmpty) none
    else
      Authorization(BasicHttpCredentials(authUser.getOrElse(""), authPassword.getOrElse(""))).some
}
