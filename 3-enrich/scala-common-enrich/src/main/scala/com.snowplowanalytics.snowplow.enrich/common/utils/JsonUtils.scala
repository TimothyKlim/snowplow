...
object JsonUtils {
  ...
  /**
    ...
    */
  val extractUnencJson: (String, String) => Validation[String, String] =
    validateAndReformatJson
  ...
  def extractJson(field: String, instance: String): Validation[String, JsonNode] =
    try {
      ...
      case e: Throwable =>
        s"Field [$field]: invalid JSON [$instance] with parsing error: ${stripInstanceEtc(
          e.getMessage).orNull}".failure
    }
