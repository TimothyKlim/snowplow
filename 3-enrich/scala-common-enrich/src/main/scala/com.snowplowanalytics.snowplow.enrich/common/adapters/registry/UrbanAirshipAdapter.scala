...
object UrbanAirshipAdapter extends Adapter {
  ...
  private val EventSchemaMap = Map(
    ...
    "IN_APP_MESSAGE_DISPLAY" -> SchemaKey("com.urbanairship.connect",
    ...
    "RICH_DELIVERY" -> SchemaKey("com.urbanairship.connect",
                                 "RICH_DELIVERY",
                                 "jsonschema",
                                 "1-0-0").toSchemaUri,
  ...
  /**
    ...
    */
  private def payloadBodyToEvent(body_json: String,
                                 payload: CollectorPayload): Validated[RawEvent] = {
  ...
  def toRawEvents(payload: CollectorPayload)(implicit resolver: Resolver): ValidatedRawEvents =
    (payload.body, payload.contentType) match {
      ...
      case (_, Some(ct)) =>
        s"Content type of ${ct} provided, expected None for ${VendorName}".failureNel
      case (Some(body), _) => {
