    ...
    def toRawEvents(payload: CollectorPayload)(implicit resolver: Resolver): ValidatedRawEvents =
      payload.body match {
        case Some(p) => {
          ...
          val schemaVersion: ValidationNel[String, String] =
            fields.size match {
              ...
              case n =>
                s"Access log TSV line contained $n fields, expected 12, 15, 18, 19, or 23".failureNel
            }
          schemaVersion.flatMap(v => {

            // Combine the first two fields into a timestamp
            val schemaCompatibleFields = "%sT%sZ"
                .format(fields(0), fields(1)) :: fields.toList.tail.tail
            ...
            val failures: ValidationNel[String, Option[RawEvent]] =
              errors match {
                case Nil => None.successNel
                case h :: t =>
                  NonEmptyList(h, t: _*).failure // list to nonemptylist
              }

            val validatedTstamp: ValidationNel[String, Option[DateTime]] =
              toTimestamp(fields(0), fields(1)).map(Some(_)).toValidationNel
            ...
            (validatedTstamp |@| failures) {
              (tstamp, e) =>
                ...
                val userAgent = schemaCompatibleFields(9) match {
                  case "" => None
                  case nonempty =>
                    ConversionUtils.singleEncodePcts(nonempty).some
                }
        ...
        case None =>
          "Cloudfront TSV has no body - this should be impossible".failureNel
      }
