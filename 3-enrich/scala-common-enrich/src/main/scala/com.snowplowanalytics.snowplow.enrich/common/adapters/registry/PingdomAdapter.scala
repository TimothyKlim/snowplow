...
object PingdomAdapter extends Adapter {
      ...
      case (qs) => {
        ...
        reformatMapParams(qs) match {
          ...
          case Success(s) => {
            ...
            s.get("message") match {
              case None =>
                s"${VendorName} payload querystring does not have 'message' as a key: no event to process".failureNel
              case Some(event) => {
  ...
  private[registry] def reformatMapParams(
      params: List[NameValuePair]): Validated[Map[String, String]] = {
    val formatted = params.map { value =>
      {
        (value.getName, value.getValue) match {
          case (k, PingdomValueRegex(v)) =>
            s"${VendorName} name-value pair [$k -> $v]: Passed regex - Collector is not catching unicode wrappers anymore".failureNel
    ...
    val failures: List[String] =
      for {
        Failure(NonEmptyList(f, _)) <- formatted
      } yield f

    (successes, failures) match {
      case (s :: ss, Nil) =>
        (s :: ss).toMap.successNel // No Failures collected.
      case (_, f :: fs) =>
        NonEmptyList(f, fs: _*).failure // Some Failures, return only those.
      case (Nil, Nil) =>
        "Empty parameters list was passed - should never happen: empty querystring is not being caught".failureNel
    }
  }
  ...
  private[registry] def reformatParameters(json: JValue): JValue =
    (json \ "action").extractOpt[String] match {
      case Some(eventType) =>
        json removeField { _ == JField("action", JString(eventType)) }
      case None => json
    }
}
