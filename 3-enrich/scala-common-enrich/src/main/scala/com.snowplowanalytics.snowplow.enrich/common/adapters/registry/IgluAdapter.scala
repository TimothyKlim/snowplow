...
object IgluAdapter extends Adapter {
  ...
  def toRawEvents(payload: CollectorPayload)(
    ...
    (params.get("schema"), payload.body, payload.contentType) match {
      case (_, Some(body), None) =>
        s"$VendorName event failed: ContentType must be set for a POST payload".failureNel
      ...
      case (Some(schemaUri), None, _) =>
        payloadToEventWithSchema(payload, schemaUri, params)
      case (_, _, _) =>
        s"$VendorName event failed: is not a sd-json or a valid GET or POST request".failureNel
    }
  }
  ...
  private[registry] def payloadSdJsonToEvent(
    ...
    contentType match {
      ...
      case "application/json; charset=utf-8" =>
        sdJsonBodyToEvent(payload, body, params)
  ...
  private[registry] def sdJsonBodyToEvent(
    ...
    parseJsonSafe(body) match {
      case Success(parsed) => {
        ((parsed \ "schema").extractOpt[String],
         (parsed \ "data").extractOpt[JObject]) match {
          ...
          case (None, _) =>
            s"$VendorName event failed: detected SelfDescribingJson but schema key is missing".failureNel
          case (_, None) =>
            s"$VendorName event failed: detected SelfDescribingJson but data key is missing".failureNel
        }
      }
