  ...
  def toRawEvents(payload: CollectorPayload)(
    ...
    (payload.api.vendor, payload.api.version) match {
      ...
      case (Vendor.UrbanAirship, "v1") =>
        UrbanAirshipAdapter.toRawEvents(payload)
      case (Vendor.Sendgrid, "v3") => SendgridAdapter.toRawEvents(payload)
      case _ =>
        s"Payload with vendor ${payload.api.vendor} and version ${payload.api.version} not supported by this version of Scala Common Enrich".failureNel
    }
