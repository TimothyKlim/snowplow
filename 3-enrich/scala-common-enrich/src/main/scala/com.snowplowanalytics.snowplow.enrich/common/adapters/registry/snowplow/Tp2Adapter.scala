...
object Tp2Adapter extends Adapter {
  ...
  private object ContentTypes {
    val list = List("application/json",
    ...
    val validatedParamsNel: Validated[NonEmptyList[RawEventParameters]] =
      (payload.body, payload.contentType) match {
        case (None, _) if qsParams.isEmpty =>
          s"Request body and querystring parameters empty, expected at least one populated".failureNel
        case (_, Some(ct)) if !ContentTypes.list.contains(ct) =>
          s"Content type of ${ct} provided, expected one of: ${ContentTypes.str}".failureNel
        case (Some(_), None) =>
          s"Request body provided but content type empty, expected one of: ${ContentTypes.str}".failureNel
        case (None, Some(ct)) =>
          s"Content type of ${ct} provided but request body empty".failureNel
  ...
  private def toParametersNel(
    ...
    (successes, failures) match {
      ...
      case (s :: ss, f :: fs) =>
        NonEmptyList(f, fs: _*).failure // Some Failures, return those. Should never happen, unless JSON Schema changed
      case (Nil, _) =>
        "List of events is empty (should never happen, did JSON Schema change?)".failureNel
    }
  }
  ...
  private def toParameter(
    ...
    Option(rawValue.textValue) match {
      ...
      case None if rawValue.isTextual =>
        s"Value for key ${key} is a null String (should never happen, did Jackson implementation change?)".failure
      case _ =>
        s"Value for key ${key} is not a String (should never happen, did JSON Schema change?)".failure
    }
  }
