...
object PagerdutyAdapter extends Adapter {
  ...
  private[registry] def payloadBodyToEvents(body: String): Validation[String, List[JValue]] =
    try {
      ...
      (parsed \ "messages") match {
        ...
        case JNothing =>
          s"${VendorName} payload does not contain the needed 'messages' key".failure
        case _ =>
          s"Could not resolve ${VendorName} payload into a JSON array of events".failure
      }
    } catch {
  ...
  private[registry] def reformatParameters(json: JValue): JValue =
    json transformField {
      ...
      case ("created_on", JString(value)) =>
        ("created_on", JString(formatDatetime(value)))
      case ("last_status_change_on", JString(value)) =>
        ("last_status_change_on", JString(formatDatetime(value)))
    }
}
