...
trait Adapter {
  ...
  private val UnstructEvent = SchemaKey("com.snowplowanalytics.snowplow",
                                        "unstruct_event",
                                        "jsonschema",
                                        "1-0-0").toSchemaUri
  ...
  private val AcceptedQueryParameters =
  ...
  private[registry] def cleanupJsonEventValues(
    ...
    eventOpt match {
      case Some((keyName, eventType)) =>
        j1 removeField { _ == JField(keyName, eventType) }
      case None => j1
    }
  }
  ...
  protected[registry] def rawEventsListProcessor(
      rawEventsList: List[Validated[RawEvent]]): ValidatedRawEvents = {
    ...
    val failures: List[String] =
      for {
        Failure(NonEmptyList(f, _)) <- rawEventsList
      } yield f

    (successes, failures) match {
      case (s :: ss, Nil) =>
        NonEmptyList(s, ss: _*).success // No Failures collected.
      case (_, f :: fs) =>
        NonEmptyList(f, fs: _*).failure // Some or all are Failures, return these.
      case (Nil, Nil) =>
        "List of events is empty (should never happen, not catching empty list properly)".failureNel
    }
  }
        ...
        eventType match {
          case et if eventSchemaMap.contains(et) => {
            eventSchemaMap.get(et) match {
              case None =>
                s"$vendor event failed: type parameter [$et] has no schema associated with it - check event-schema map".failureNel
              case Some(schema) => schema.success
            }
          }
          case "" =>
            s"$vendor event failed: type parameter is empty - cannot determine event type".failureNel
          case et =>
            s"$vendor event failed: type parameter [$et] not recognized".failureNel
        }
      }
    }
      ...
      case Some(eventType) => {
        eventType match {
          case et if eventSchemaMap.contains(et) => {
            eventSchemaMap.get(et) match {
              case None =>
                s"$vendor event at index [$index] failed: type parameter [$et] has no schema associated with it - check event-schema map".failureNel
              case Some(schema) => schema.success
            }
          }
          case "" =>
            s"$vendor event at index [$index] failed: type parameter is empty - cannot determine event type".failureNel
          case et =>
            s"$vendor event at index [$index] failed: type parameter [$et] not recognized".failureNel
        }
      }
    }
