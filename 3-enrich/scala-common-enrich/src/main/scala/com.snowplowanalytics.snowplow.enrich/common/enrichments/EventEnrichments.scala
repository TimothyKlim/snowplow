...
object EventEnrichments {
  ...
  /**
    ...
    */
  def fromTimestamp(timestamp: String): DateTime =
    TstampFormat.parseDateTime(timestamp)
  ...
  def getDerivedTimestamp(
    ...
    case None =>
      try {
        ...
        case NonFatal(e) =>
          s"Exception calculating derived timestamp: [$e]".failure
      }
  }
  ...
  val extractTimestamp: (String, String) => ValidatedString = (field, tstamp) =>
    try {
      ...
      case nfe: NumberFormatException =>
        "Field [%s]: [%s] is not in the expected format (ms since epoch)"
          .format(field, tstamp)
          .failure
  }
