...
case class Input(key: String, pojo: Option[PojoInput], json: Option[JsonInput]) {
  ...
  def getFromEvent(event: EnrichedEvent): TemplateContext = pojo match {
    case Some(pojoInput) => {
      try {
        ...
        case NonFatal(err) =>
          s"Error accessing POJO input field [$key]: [$err]".failureNel
      }
    }
  ...
  def getFromJson(derived: List[JObject],
                  custom: List[JObject],
                  unstruct: Option[JObject]): TemplateContext =
    json match {
      case Some(jsonInput) => {
        val validatedJson: ValidationNel[String, Option[JValue]] =
          jsonInput.field match {
            case "derived_contexts" =>
              getBySchemaCriterion(derived, jsonInput.schemaCriterion).successNel
            case "contexts" =>
              getBySchemaCriterion(custom, jsonInput.schemaCriterion).successNel
            case "unstruct_event" =>
...
object Input {
  ...
  type TemplateContext =
    ValidationNel[String, Option[Map[String, String @@ Tags.LastVal]]]
