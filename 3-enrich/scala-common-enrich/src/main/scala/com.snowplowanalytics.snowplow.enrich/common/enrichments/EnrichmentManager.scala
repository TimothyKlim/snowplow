...
object EnrichmentManager {
  ...
  def enrichEvent(registry: EnrichmentRegistry,
    ...
    val currency = {
      registry.getCurrencyConversionEnrichment match {
        case Some(currency) => {
          ...
          val trTax =
            CU.stringToMaybeDouble("tr_tx", event.tr_tax).toValidationNel
          val tiPrice =
            CU.stringToMaybeDouble("ti_pr", event.ti_price).toValidationNel
          val trTotal =
            CU.stringToMaybeDouble("tr_tt", event.tr_total).toValidationNel
          val trShipping =
            CU.stringToMaybeDouble("tr_sh", event.tr_shipping).toValidationNel
    ...
    val pageQsMap = pageUri match {
      case Success(Some(u)) =>
        CU.extractQuerystring(u, raw.source.encoding).map(_.some)
      case _ => Success(None)
    }
    ...
    // This enrichment cannot.failure
    (registry.getEventFingerprintEnrichment match {
      case Some(efe) =>
        event.event_fingerprint = efe.getEventFingerprint(sourceMap)
      case _ => ()
    })

    // Validate custom contexts
    val customContexts =
      Shredder.extractAndValidateCustomContexts(event) match {
    ...
    val httpHeaderExtractorContext =
      registry.getHttpHeaderExtractorEnrichment match {
        case Some(hee) =>
          val headers = raw.context.headers
