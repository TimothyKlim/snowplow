...
object CljTomcatLoader extends Loader[String] {
  ...
  def toCollectorPayload(line: String): ValidatedMaybeCollectorPayload = {
    ...
    def build(qs: String,
      ...
      val querystring =
        parseQuerystring(CloudfrontLoader.toOption(qs), CollectorEncoding)
      val timestamp = CloudfrontLoader.toTimestamp(date, time)
      val contentType = (for {
        enc <- ct
        raw = ConversionUtils.decodeString(CollectorEncoding,
                                           "Content type",
                                           enc)
      } yield raw).sequenceU
      val body = (for {
    ...
    line match {
      ...
      case CljTomcatRegex(_, _, _, _, _, op, _, _, _, _, _, _, _, _)
          if op.toUpperCase != "POST" =>
